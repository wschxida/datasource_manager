...
fhlr = logging.FileHandler('./log/clear_reject_domain.log')  # 输出到文件的handler
...
def main():
    ...
    for domain in reject_domain_list:
        # sql = f'''select count(1) as count from {table} where domain_code='{domain_code}';'''
        sql = f'''delete from {table} where domain_code='{domain}';'''
        result = query_mysql(database_config, sql)
        # print(sql)
        logging.info(sql)
        # print('delete count: ', result)
