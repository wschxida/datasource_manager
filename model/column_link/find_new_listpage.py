...
fl_handler = RotatingFileHandler(logFile, mode='a', maxBytes=5*1024*1024, backupCount=20, encoding='utf-8', delay=0)
...
def parse_html_to_database(database_config, url, column_extraction_deep, domain_code_source, website_no, Is_Need_VPN, text):
    try:
        ...
        for num, item in enumerate(items):
            ...
            logger.debug(str(num) + 'percent:{:.0%}'.format(node_percent) + title)
            if len_items < 50:
                if node_percent > 0.5:
                    continue
...
async def get_response(database_config, semaphore, url, column_extraction_deep=1, domain_code_source=None, website_no=None, Is_Need_VPN=0):
    try:
        async with semaphore:
            ...
            connector = aiohttp.TCPConnector(limit=60, force_close=True, verify_ssl=False, enable_cleanup_closed=True)  # 60小于64。也可以改成其他数
