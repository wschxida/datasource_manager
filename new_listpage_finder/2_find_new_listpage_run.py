...
fhlr = logging.FileHandler('./log/listpage_url_pusher.log', encoding='utf-8')  # 输出到文件的handler
    ...
    try:
        ...
        for num, item in enumerate(items):
            ...
            if level_score >= valid_score:
                column = f"('{title}', '{listpage_url}', '{record_md5_id}', '{website_no}', {Is_Need_VPN}, {column_extraction_deep}, '{domain_code}', '{host_code}', '{level_score}', '{score_detail}')"
                column_list.append(column)

        # 批量插入
        values = ",".join(column_list)
        insert_column = f"insert ignore into new_listpage_url_cache(Listpage_Title, Listpage_URL, Listpage_URL_MD5, Website_No, Is_Need_VPN, Column_Extraction_Deep, Domain_Code, Host_Code, Level_Score, Score_Detail) values{values};"
        # print(insert_column)
    ...
    try:
        ...
        for item in target_items:
            ...
            if column_extraction_deep <= 3:
                task = asyncio.ensure_future(get_response(database_config, semaphore, url, column_extraction_deep, domain_code, website_no, Is_Need_VPN))
                tasks.append(task)
...
def main():
    ...
    for i in range(10000000):
        try:
            target_count = create_task(event_loop, semaphore, database_config)
            print(target_count)
            if target_count < 1:
                # break
                time.sleep(10)
